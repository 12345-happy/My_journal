...
export function Modify() {
  ...
  const deletePost = () => {
    console.log('Delete button clicked') // 로그 추가로 함수 호출 확인

    // 확인 창에서 확인을 받으면 삭제 진행
    if (window.confirm('정말로 삭제하시겠습니까?')) {
      const url = `http://localhost:8080/apiserver/journal/remove/${jno}`

      // body에 PageRequestDTO에 해당하는 정보 추가
      const bodyData = {
        page: page,
        type: type,
        keyword: keyword
      ...
      fetch(url, {
        ...
        body: JSON.stringify(bodyData) // 요청 본문에 데이터 추가
      })
        .then(res => res.json())
        .then(data => {
          if (data.msg) {
  ...
  function deleteFileDb(fileName, uuid) {
    const removeUrl = `http://localhost:8080/apiserver/removeFile?fileName=${encodeURIComponent(
      fileName
    ...
    fetch(removeUrl, {
      ...
      headers: {
        Authorization: `Bearer ${token}`,
        'Content-Type': 'application/json' // 서버가 JSON을 기대할 경우
      }
    })
      .then(response => response.json())
      .then(json => {
        if (json.success === true) {
          console.log(`${fileName} 파일이 성공적으로 삭제되었습니다.`)
          // 파일이 삭제된 후 UI에서 해당 파일을 제거
        } else {
          console.log(
            `파일 ${fileName} 삭제에 실패했습니다. 서버 메시지: ${
